...
| `status` | string | Да | Статус ТД: "ТД Зарегистрирована" или "Транзит завершен" (регистронезависимо) |
...
Поддерживаются два статуса (регистронезависимо):
...
При получении статуса "Транзит завершен" обновляются только:

1. **status_td** (ID: 984685) - значение "Транзит завершен", передаваемое в amoCRM точным текстом
2. **completion_date_td** (ID: 990795) - Дата и время завершения транзита

Номер ТД и дата регистрации ТД при завершении транзита не изменяются.

### Изменение статуса сделки

Маршрутизация зависит от воронки найденной сделки:

| Воронка | `ТД Зарегистрирована` | `Транзит завершен` |
|---------|------------------------|--------------------|
| Основная (ID `7523034`) | Обновить поля; перейти на `Транзит` (ID `83281430`) только с разрешённых этапов | Обновить поля; этап не менять |
| Транзит по РФ (ID `10944982`) | Обновить поля; перейти на `Транзит по РФ (открыт)` (ID `86060670`) | Обновить поля; перейти на `Транзит по РФ (закрыт)` (ID `86372326`) |
| Офис Москва (ID `11036918`) | Игнорировать событие | Игнорировать событие |

В основной воронке при регистрации ТД статус сделки изменяется на **"Транзит"** (ID `83281430`) **только если**:
...
При статусе "Транзит завершен" карточка основной воронки не перемещается независимо от её текущего этапа.

Закрытая сделка в воронке «Транзит по РФ» не открывается повторно при поздно пришедшем событии `ТД Зарегистрирована`.

Если один VIN найден сразу в нескольких подключённых воронках, запрос завершается ошибкой до обновления полей и этапов: интеграция не выбирает сделку произвольно.
...
```json
{
  ...
  "status": "Транзит завершен",
  "statusDate": "2026-07-17 12:24:26"
}
```

**Что произошло:**
- Обновлены `status_td` и `completion_date_td`
- Номер ТД и дата регистрации не изменены
- Карточка сделки не перемещена

### Пример 4: Формат даты с точкой
...
```json
{
  "error": "Статус 'Неизвестный статус' не поддерживается. Ожидается: 'ТД Зарегистрирована' или 'Транзит завершен'"
}
```
...
Log::info("TD status update: статус сделки НЕ изменен", [
    ...
    'reason' => 'завершение транзита не меняет этап основной воронки'
]);
```
