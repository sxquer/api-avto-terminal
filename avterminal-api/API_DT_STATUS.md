...
- В обычном режиме: ищет сделку по VIN номеру в основной воронке, «Транзит по РФ» и «Офис Москва»
- Если один VIN найден в нескольких подключённых воронках, обработка завершается ошибкой до любых изменений

### 1.1. Маршрутизация по воронкам

| Воронка | Обрабатываемые статусы | Результат |
|---------|-------------------------|-----------|
| Основная (ID `7523034`) | Все ранее поддержанные статусы | Сохраняется существующая логика полей, этапов, истории и защиты от отката |
| Офис Москва (ID `11036918`) | `регистрация ПТД` | Обновить поля; перейти на `ПТД/ТД` (ID `86718954`) |
| Офис Москва (ID `11036918`) | `выпуск с уплатой` | Обновить поля; перейти на `Выпуск по ПТД/ДТ` (ID `86718958`) |
| Офис Москва (ID `11036918`) | Остальные статусы ДТ | Игнорировать событие |
| Транзит по РФ (ID `10944982`) | Все статусы ДТ | Игнорировать событие |

Повторное событие не вызывает повторной смены этапа. `регистрация ПТД` не откатывает сделку «Офис Москва», уже находящуюся на этапе выпуска.
...
ID стадий основной воронки из конфига:
- `ПТД/ДТ`: 62360974
- `Выпуск ПТД/ДТ`: 81192786

### Защита от излишнего "отката" сделок
Если сделка находится на одном из "защищенных" этапов, то при получении статусов:
- `регистрация ПТД`
- `выпуск без уплаты`
- `выпуск с уплатой`
- `отказ в выпуске товаров`
...
**Сделка не будет переведена на стадии `ПТД/ДТ` или `Выпуск ПТД/ДТ`**, но данные ДТ будут обновлены, а информация о защите записана в логи.
