<?php

return [
    'client_id' => env('AMOCRM_CLIENT_ID'),
    'client_secret' => env('AMOCRM_CLIENT_SECRET'),
    'subdomain' => env('AMOCRM_SUBDOMAIN'),
    'long_lived_token' => env('AMOCRM_LONG_LIVED_TOKEN'),

    // Тестовый режим для API метода updateDtStatus
    // В тестовом режиме всегда используется сделка с ID 25147637
    'test_mode' => env('AMOCRM_TEST_MODE', false),

    'fields' => [
        'vin_field_id' => [
            'id' => 808681,
        ],

        'color_field_id' => [
            'id' => 974799,
            'values' => [
                'Синий' => 1231085,
                'Красный' => 1231087,
                'Зеленый' => 1231089,
            ],
        ],

        'uss_invoice_issued' => [
            'id' => 990217,
        ],

        'uss_invoice_paid' => [
            'id' => 990219,
        ],

        'nomer_dt' => [
            'id' => 979423,
        ],

        'status_dt' => [
            'id' => 979425,
            'values' => [
                'регистрация ПТД' => 1234079,
                'выпуск без уплаты (10)' => 1234081,
                'требуется уплата (31)' => 1234083,
                'выпуск с уплатой (32)' => 1234085,
                'выпуск разрешен, ожидание решения по временному ввозу (33)' => 1234087,
                'отказ в разрешении (40)' => 1234089,
                'выпуск товаров аннулирован при отзыве ПТД (50)' => 1234091,
                'отказ в выпуске товаров (90)' => 1234093,
            ],
        ],

        'registration_date' => [
            'id' => 979427,
        ],

        'vipusk_date' => [
            'id' => 979429,
        ],

        'refuse_date' => [
            'id' => 979431,
        ],

        'history' => [
            'id' => 979433,
        ],

        'nomer_td' => [
            'id' => 984681,
        ],

        'status_td' => [
            'id' => 984685,
            'values' => [
                'ТД Зарегистрирована' => 1238357,
                // amoCRM API может определить enum по точному тексту значения.
                'Транзит завершен' => 'Транзит завершен',
            ],
        ],

        'registration_date_td' => [
            'id' => 984687,
        ],

        'completion_date_td' => [
            'id' => 990795,
        ],

        'onec_counterparty_id' => [
            'id' => 986431,
        ],

        'warehouse' => [
            'id' => 969469,
        ],

        'car_model' => [
            'id' => 808675,
        ],

        'car_brand' => [
            'id' => 808679,
        ],

        'contract_date_company' => [
            'id' => 919495,
        ],

        'contract_date_contact' => [
            'id' => 947713,
        ],

    ],

    // Статусы из которых при полученнии ДТ переходим в статус Транзит (83281430)
    'td_statuses_to_change' => [
        62360714,
        62360726,
        62360954,
    ],

    'td_transit_status' => 83281430,

    'statuses' => [
        'ptd/dt' => 62360974,
        'vipusk' => 81192786,
        'svh' => 64976646,
    ],

    'pipelines' => [
        'main' => [
            'id' => 7523034,
            'statuses' => [
                'td_registration' => 83281430,
                'dt_registration' => 62360974,
                'dt_release' => 81192786,
            ],
        ],
        'transit_russia' => [
            'id' => 10944982,
            'statuses' => [
                'td_registration' => 86060670,
                'td_completion' => 86372326,
            ],
        ],
        'office_moscow' => [
            'id' => 11036918,
            'statuses' => [
                'dt_registration' => 86718954,
                'dt_release' => 86718958,
            ],
        ],
    ],

    'onec' => [
        'contract_pipeline_id' => env('ONEC_CONTRACT_PIPELINE_ID', 7523034),
        'contract_status_id' => env('ONEC_CONTRACT_STATUS_ID', 62360726),
        'payment_pipeline_id' => env('ONEC_PAYMENT_PIPELINE_ID', 7523034),
        'webhook_secret' => env('ONEC_AMO_WEBHOOK_SECRET'),
        'test_webhook_secret' => env('ONEC_AMO_WEBHOOK_TEST_SECRET'),
    ],
];
